...
Three things remain.
(i) The throughput claim moved down when it was repeated: a single run had given $110\,610$, the median of five gives $94\,139$, so it is $1.28\times$ Fhenix and not $1.5\times$, and the single-threaded figure no longer beats them. If you quote a number anywhere else in the paper, quote this one.
(ii) Neither machine is named. Fhenix names a \texttt{c7a.32xlarge}; this section should name its counterpart, and it matters more than usual here because one host carries all $N$ parties in our setup and the $N = 16$ rows are visibly limited by that.
(iii) I recomputed Fhenix's $73\,578$ from the logs in \texttt{benchmarks/}, not from its tables, aggregating total ciphertexts over the slowest concurrent job. Check one cell against the published table before building on it; the $1.28\times$ and the latency comparison both rest on that denominator.
}
